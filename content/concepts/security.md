...
The blast radius is bounded on three axes. A notification cap (default 20; `0` selects the default rather than unlimited) bounds how much spam a misbehaving agent can generate, but the bound is per `sortie mcp-server` process, not per session: an agent runtime that keeps one process running for the whole session is bounded at 20, but a runtime that starts a fresh process every turn resets the count with it, so the true ceiling for that kind of runtime scales with the number of turns. A 10-second per-call timeout bounds how long a slow endpoint can stall a turn. And delivery stops at the first failing backend instead of working through the rest of the list.
