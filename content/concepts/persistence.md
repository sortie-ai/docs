...
**Run history.** One row per completed worker session: issue ID, identifier, exit type, start and completion timestamps, agent adapter used, workspace path, and token counts (input, output, total, cache read). This solves three problems. First, `max_sessions` budget enforcement: the count of rows for an issue is the count of sessions spent, durable across any number of restarts. Second, debugging: when an agent fails at 2 AM, the run history tells you what happened without digging through logs. Third, this is the raw material for cost attribution — every session records how many tokens it consumed.
...
**Cost attribution.** Run history already stores per-session token counts, and session metadata stores the model name for each session. Enterprise cost dashboards, per-issue cost reports, and budget alerts can all read from these tables. The data collection happens today in the free core; the reporting surfaces come in future milestones.
