...
Translation buys reach at the cost of one boundary it will not cross. Both translated forms live in the launch itself, not in a file the remote host can be handed, and neither adapter builds that form for a remote launch. For Codex the reason is concrete: the only route left is the remote command string, which is the local `ssh` process's own argument list, readable by every other user of the orchestrator host, and the generated configuration carries the tracker credential. Sortie declines to publish it. A Codex or OpenCode session dispatched to an SSH host reaches no tools at all.
