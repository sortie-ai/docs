...
A related protection covers the `.sortie` control directory once the workspace already exists. Every file Sortie itself writes there (the MCP tool-server configuration, worker state, the dispatch identity record) opens the workspace root, creates a fresh temporary file, and renames it into place, rather than opening the destination name directly; a `.sortie` that turns out to be a symlink, or that is not a directory, is refused instead of followed. The write that generates `.sortie/mcp.json` runs before the agent session starts, so a workspace whose `.sortie` is a symlink at that point fails the run before any agent code executes. The other writes happen over the course of the run and are not fatal the same way: a rejected write there is logged and skipped, and the run continues.
