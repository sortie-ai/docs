...
Before launching any agent subprocess, Sortie re-resolves and re-validates the workspace path (confirms it is non-empty, resolves cleanly to absolute form, still exists, and is still a directory) and only then hands that freshly-checked path to the subprocess as its working directory. This check runs immediately before every launch that uses the workspace as its working directory, not just once at session start: every turn, every hook, the self-review verification command, and Sortie's own git inspection all re-check the path right before they start. If someone or something moved the directory, swapped it with a symlink, or removed it entirely between workspace creation and any one of those launches, that step fails instead of following the link or continuing as if nothing were wrong. Workspaces persist across sessions for the same issue: a retry reuses the same directory, so agents can build on partial commits, cached dependencies, and compilation artifacts from earlier attempts. The [orchestration model](/concepts/orchestration/) explains how retries interact with workspace lifecycle.

A related protection covers the `.sortie` control directory once the workspace already exists. Every write, read, and removal Sortie performs there (the MCP tool-server configuration, worker state, the dispatch identity record, the status file, the SCM metadata) opens the workspace directory and then `.sortie` inside it as a pair of verified handles, re-confirming immediately after each open that what it opened still matches what it checked a moment before; a write also creates a fresh temporary file and renames it into place rather than opening the destination name directly. The workspace directory or `.sortie` turning out to be a symlink, the wrong type, or swapped out at the exact moment Sortie opens it is refused instead of followed. The write that generates `.sortie/mcp.json` runs before the agent session starts, so a workspace whose directory or `.sortie` fails that check at that point fails the run before any agent code executes. The other writes, reads, and removals happen over the course of the run and are not fatal the same way: a rejected operation there is logged and skipped, and the run continues.
