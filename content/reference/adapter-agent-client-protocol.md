...
A source supplies a figure only when two checks both pass. It has to claim the launch, and none does for a session running over SSH, because it reads output the runtime writes on the machine that ran it. And the handshake has to name a build that source was measured against, so upgrading the runtime can end measurement with nothing else about the run changing. Either check failing leaves the session unmeasured for the whole of its life. Which way a session went is observable rather than inferred: the once-per-session capability notice names `token counts` exactly when no figure will arrive, described under [capability tracking](#capability-tracking). Sortie ships one such source today, for Gemini CLI.
