...
| `-dumpversion` | boolean | `false` | Print the bare version string (e.g., `1.11.0`), then exit. |
...
```
time=2026-04-07T14:30:00.000+00:00 level=INFO msg="sortie starting" version=1.11.0
```

When `json` is active, each log line is a single JSON object via `slog.JSONHandler`:

```json
{"time":"2026-04-07T14:30:00.000Z","level":"INFO","msg":"sortie starting","version":"1.11.0"}
```
...
```
sortie 1.11.0 (commit: d5a8ed1, built: 2026-04-26, go1.26.1, linux/amd64)
```
...
```
time=2026-03-26T14:30:01.271+00:00 level=INFO msg="sortie starting" version=1.11.0 workflow_path=/opt/sortie/WORKFLOW.md port=8080
...
```sh
go build -ldflags "-s -w -X main.Version=1.11.0" -o sortie ./cmd/sortie
```
