...
- The `max_tokens` token budget is reached, either at a dispatch decision, or by the in-flight check when its cancellation is what ends the running session.
...
| `LaunchingAgentProcess` | Before the agent adapter starts the working session, the [credential-verification step](/reference/workflow-config/#credential-verification) opens a session of its own, sends one fixed request, and closes it again, on every agent kind and every host, including one from [`worker.ssh_hosts`](/reference/workflow-config/#worker). A runtime that cannot complete that request ends the attempt here with error kind `credential_unverified`, or with `port_exit` when it exits before responding (the [early exit report](/reference/errors/#early-exit-report)), before the working session ever starts. Once that check passes, the agent adapter starts the working session (subprocess or API call). |
...
| **Worker exit (cancelled)** | The worker was cancelled by reconciliation, by stall detection, by the `agent.max_tokens` in-flight check, or by shutdown. Remove the issue's running-session entry. Persist run history, under status `budget_stopped` when the token ceiling's own cancellation ends the run and `cancelled` otherwise. See [how to control agent costs](/guides/control-costs/#cap-tokens-per-issue). Release the claim only when no retry is already queued: a retry pre-scheduled by stall detection keeps the claim so nothing else can dispatch the issue. No handoff transition, no new retry. |
| **Agent update event** | Update the issue's running-session entry: token counters, session ID, thread ID, agent PID, rate limits, last activity timestamp. This still applies to a run whose worker has already exited, as long as that exit has not yet been handled, so the event never lands on a later run of the same issue. An event carrying token usage evaluates the issue against `agent.max_tokens` and cancels the worker once the sum reaches it, unless the event is still waiting when that issue's worker exit is taken up: then the usage reaches the next dispatch decision through the run's recorded total instead. During shutdown drain, no event is evaluated against the ceiling at all. |
...
| **Self-review progress** | Marks the running entry as self-reviewing and records the iteration number, or clears both when the review loop ends. A progress message for a run whose worker has already exited is applied to that run before the exit is handled, so it never lands on a later run of the same issue. Live session bookkeeping only; no claim or retry transition. |
