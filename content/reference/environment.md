...
| `SORTIE_AGENT_TOKEN_WARNING_PERCENT` | [`agent.token_warning_percent`](/reference/workflow-config/#agent) | int |
...
| `KIRO_API_KEY` | `kiro` adapter, unless a stored `kiro-cli` login already authenticates it | API key the Kiro CLI reads on the headless path. Every run's [credential-verification step](/reference/workflow-config/#credential-verification) runs a `kiro-cli whoami` guard, on a session of its own, before any working turn; a missing or invalid credential surfaces there, before any work starts, rather than as a hang or a silent empty turn. |

**A missing `ANTHROPIC_API_KEY` is the most common `claude-code` deployment failure.** Sortie starts and polls the tracker normally, but every agent session fails at credential verification with an auth error. The Sortie logs show a worker exit with `exit_type=error`; the root cause is only visible in the agent's stderr output.

**For `copilot-cli`, a missing GitHub token is the equivalent failure.** The Copilot CLI itself implements try-and-fallback across `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, and `GITHUB_TOKEN`; precedence matters only when multiple sources hold different valid tokens. Sortie runs no preflight of its own here: whether one of these variables resolves to a working credential is settled by the [credential-verification step](/reference/workflow-config/#credential-verification), which fails before any work starts when none of them does: with error kind `credential_unverified`, or with the [early exit report](/reference/errors/#early-exit-report) under `port_exit`, carrying the CLI's own message, when the CLI prints that message and exits.
...
**For `opencode`, authentication is provider-specific and the adapter does not preflight it.** OpenCode resolves credentials from its own environment, auth store, project `.env`, or `opencode.json` provider config, while the Sortie adapter injects or overrides a small managed `OPENCODE_*` set on every `run` and usage-recovery subprocess. The adapter supports OpenCode 1.x and 2.x, detected at the start of each session, and manages a different variable set for each; see the [OpenCode adapter reference](/reference/adapter-opencode/#version-detection).

| Variable | Purpose | Description |
|---|---|---|
| `OPENCODE_PERMISSION` | Permission policy (1.x only) | Inline JSON permission policy. When `opencode.allowed_tools` or `opencode.denied_tools` is configured, Sortie removes any inherited value and writes its managed policy instead. On 2.x the same policy travels inside `OPENCODE_CONFIG_CONTENT` below rather than as its own variable. |
| `OPENCODE_AUTO_SHARE` | Session sharing (1.x only) | Auto-share on completion. Sortie-managed runs force this to `false`. On 2.x the same setting is the `share` member of `OPENCODE_CONFIG_CONTENT`, always `"disabled"`. |
| `OPENCODE_DISABLE_AUTOCOMPACT` | Context compaction (1.x only) | Managed by `opencode.disable_autocompact`. On 2.x the same setting is the `compaction.auto` member of `OPENCODE_CONFIG_CONTENT`. |
| `OPENCODE_DISABLE_AUTOUPDATE` | Self-update | Sortie-managed runs force this to `true`, on both majors. |
| `OPENCODE_DISABLE_LSP_DOWNLOAD` | LSP download (1.x only) | Sortie-managed runs force this to `true`. OpenCode 2.x has no equivalent setting. |
| `OPENCODE_CONFIG_CONTENT` | Inline configuration (2.x), MCP servers (both) | On 1.x, set only when the session has tool servers to deliver, carrying just those servers. On 2.x, set on every local turn, carrying the permission policy, sharing, compaction, and tool servers together. Never set on a remote launch. |

In local mode the adapter injects only the managed `OPENCODE_*` values above; every provider credential, and any OpenCode config-discovery variable such as `OPENCODE_CONFIG`, comes from the parent environment or from OpenCode's own auth and config state, unmanaged by Sortie. A remote session receives the managed variables for the detected major and nothing else of Sortie's own, and never `OPENCODE_CONFIG_CONTENT`, so the provider credentials your model selection needs must already exist on the remote host, or be named under [`worker.ssh_pass_env`](/reference/workflow-config/#environment-variables-carried-to-a-remote-agent).

**For `kiro`, authentication is `KIRO_API_KEY` or a stored `kiro-cli` login.** The [credential-verification step](/reference/workflow-config/#credential-verification) runs a `kiro-cli whoami` guard, on a session of its own, before any working turn; a missing or invalid credential surfaces there rather than as a hang or a silent empty turn on the working session. That guard runs the same way on a local launch and on a remote one: a remote session still carries `KIRO_API_KEY` from Sortie's environment into the agent's environment on the host, and the guard runs against whatever credential is present there. See the [Kiro CLI adapter reference](/reference/adapter-kiro/) for the guard's own mechanism and the rest of the headless behavior.
...
2. **Per-session variables** (higher precedence). The worker writes the following variables, overriding any same-named key from layer 1:
...
The agent's session identifier is not one of these variables. Sortie writes the tool server's environment before that identifier is known, so an environment variable would always read empty. Instead, the `notify_operator` tool reads the session id live, at call time, from a `.sortie/dispatch.json` record the worker keeps current for the running dispatch, matched against `SORTIE_DISPATCH_ID`. See [`notify_operator`](/reference/agent-extensions/#notify_operator) for how that record is populated and when the session id it reports is empty.
