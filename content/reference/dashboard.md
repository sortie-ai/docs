...
|---|---|---|
| A running session's agent kind reports usage, none has arrived, and one still can | "1 running session has not reported token usage yet; the totals above exclude it." | "N running sessions have not reported token usage yet; the totals above exclude them." |
| A running session's agent kind reports no token usage at all, or reports usage and can no longer produce a figure for this session | "1 running session runs an agent that reports no token usage; the totals above exclude it." | "N running sessions run agents that report no token usage; the totals above exclude them." |
