...
| `read_timeout_ms` | integer | `5000` (5 seconds) | Bounds the wait for the turn's first JSON envelope, and, doubled and capped at 30 seconds, the post-turn `export` and `models` subprocesses and, on a [credential-verification](/reference/workflow-config/#credential-verification) session, the `session delete` cleanup call. It does not bound anything after the first envelope arrives. Falls back to 30 seconds when unset or not positive. |
...
6. On a [credential-verification](/reference/workflow-config/#credential-verification) session that reached a session ID, runs `opencode session delete <id>` in the workspace before returning. A failed delete, including exit `1` for an identifier the runtime does not recognize, is only logged; it never fails the run.
