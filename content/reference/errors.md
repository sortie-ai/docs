...
| Error kind | Description | Retryable | Backoff | Operator action |
|---|---|---|---|---|
| `agent_not_found` | Agent command or binary not found in `PATH`. An exit code `127` reports this kind only once the agent has written output; an agent that exits `127` before it responds, a remote binary missing from `PATH` included, reports the [early exit report](#early-exit-report) under `port_exit` instead. | No | - | Install the agent binary, or set `agent.command` in WORKFLOW.md. For SSH workers, install the agent on the remote host. |
...
| `port_exit` | Agent subprocess exited unexpectedly (non-zero exit code, pipe failure, or crash), or the runtime reported no turn outcome and the adapter had no per-turn process exit to observe. An agent that exits before it responds, whatever its exit status, reports the [early exit report](#early-exit-report): its exit status and the end of what it wrote to standard error. | Yes | Exponential | Read the message. An early exit report quotes the agent's own explanation, such as a switch in `agent.command` it does not accept, a credential it rejects, or an interpreter it cannot find. For SSH workers, exit code `255` indicates an SSH connection failure. Check connectivity and verify the host is in `worker.ssh_hosts`. |
...
| `credential_unverified` | Before the working session's first turn, every worker attempt sends one fixed request through a short-lived session of its own to confirm the agent's credential actually works; the runtime refused it, never answered it, or otherwise failed to complete it. See [credential verification](/reference/workflow-config/#credential-verification) for what runs and when. | Yes | Exponential | Read the message: it names why the check failed. A stored login or API key that has expired or been revoked needs refreshing; see the [environment variables reference](/reference/environment/#agent-runtime-variables) for what each kind reads. |

This same check can also fail under an error kind of its own instead of `credential_unverified`. A failure that keeps the check from even starting its session, a missing agent binary (`agent_not_found`) or an invalid workspace path (`invalid_workspace_cwd`) most commonly, reports that kind unchanged, because the problem has nothing to do with the credential. An agent that exits before it responds reports the [early exit report](#early-exit-report) under `port_exit`, including one that rejects its credential only by printing a message and exiting: the message it printed is the diagnosis. And once the check's one request is running, a failure whose own kind is non-retryable (for example `turn_refused` or `turn_token_limit`) or that is specifically an SSH connection failure (`port_exit`) also keeps that kind, because it already says something more specific than a credential failure would. Every other way the request can fail to complete becomes `credential_unverified`: a retryable failure of a different kind, and, regardless of what else was happening, the request simply taking too long to answer.

Failure text is uniform across coding agents. A turn that exits `0` having written output, but no evidence that the model did any work, reports `agent exited without producing output`, followed by the signals the adapter looked for and did not find. An adapter that reads both model-authored content and tool-call activity reports `agent exited without producing output: no message from the agent and no tool call`, which covers `claude-code`, `copilot-cli`, and `opencode`. `kiro` counts any readable line of its transcript as a message from the agent, so a `kiro` turn with nothing to show takes the early exit report instead. A non-zero exit after the agent wrote output reports `exit code N`. A runtime that reported no turn outcome and gave the adapter no process exit to observe reports `runtime reported no turn outcome`. A runtime whose protocol defines a task-completion report, but that ended the turn without one, reports `agent stopped without reporting the task complete`.

### Early exit report

An agent that exits before it responds is reported the same way on every agent kind, on the credential check and on a working turn alike. The error kind is `port_exit`, retried with exponential backoff, and the message reads `the agent runtime exited before responding: <status>`. `<status>` is `exit status N`, or `signal: <name>` for a process ended by a signal Sortie did not send. The rest of the error text is the end of what the agent wrote to standard error:

```
agent: port_exit: the agent runtime exited before responding: exit status 2: error: unknown option '--sortie-unknown-switch'
```

| Property | Value |
|---|---|
| Line separator | A pipe with a space on either side, between the standard-error lines, which are joined into one line |
| Size bound | The last 8 KiB of that line |
| Cut marker | A leading `[earlier output omitted]` when the bound, or the collector's own line limit, dropped earlier output |
| Nothing readable | `no standard-error output` in place of the text |
| Cleaning | Escape sequences and control characters removed; known credentials shown as `[redacted]` (see [secrets and credential handling](/concepts/security/#secrets-and-credential-handling)) |
| SSH connection failure | A remote launch whose `ssh` exits `255` reports the SSH connection failure instead |

What counts as responding depends on how the agent kind runs its runtime. A kind that starts a new process for every turn has responded once it writes a line on standard output that its adapter accepts as a response: a line the adapter's decoder parses as a protocol message, for a kind with a structured output format (`claude-code`, `copilot-cli`, `opencode`), or any readable line, for a kind whose output is plain text (`kiro`). A line the decoder rejects is not a response, so a turn whose process exits before an accepted line takes this report whatever its exit status, `0` and `127` included, even if it wrote other, unreadable output first. A kind that keeps one process for the whole session has responded once its startup handshake completes, so a runtime that exits during the handshake takes this report. Each agent adapter reference names its kind's subprocess model and output format. A process Sortie stopped itself, on cancellation, a timeout, or shutdown, is never reported this way.
...
| `cancelled` | Cancelled (reconciliation kill, stall detection, the [`agent.max_tokens`](/reference/workflow-config/#agent) in-flight check, or shutdown). | Claim released unless reconciliation pre-scheduled a retry. No automatic retry: reconciliation handles re-dispatch. The run is recorded `cancelled`, except when the token ceiling's own cancellation ends it, which is recorded `budget_stopped` with the tokens used and the ceiling in its error text. See [how to control agent costs](/guides/control-costs/#cap-tokens-per-issue). |
...
| `127` | `port_exit` | Remote agent binary not found in `PATH`, reported as the [early exit report](#early-exit-report): `exit status 127` followed by the remote shell's own message. | Yes (exponential) | Install the agent on the remote host. Verify `PATH` for the SSH user. |
