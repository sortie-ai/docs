...
What counts as responding depends on how the agent kind runs its runtime. A kind that starts a new process for every turn has responded once the process writes a readable line to standard output, so a turn whose process exits before that line takes this report whatever its exit status, `0` and `127` included. A kind that keeps one process for the whole session has responded once its startup handshake completes, so a runtime that exits during the handshake takes this report. Each agent adapter reference names its kind's subprocess model. A process Sortie stopped itself, on cancellation, a timeout, or shutdown, is never reported this way.
