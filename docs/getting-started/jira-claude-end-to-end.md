...
- [Jira integration tutorial](jira-integration.md) completed — Sortie connects to your Jira project, and the environment variables `SORTIE_JIRA_ENDPOINT` and `SORTIE_JIRA_API_KEY` are set
...
If the status did not change and you see a handoff warning in the logs, the Jira workflow does not allow a direct transition from "To Do" to "Done." Check the [Jira integration tutorial](jira-integration.md#verify-in-jira) troubleshooting section for how to resolve this.
...
- [Write a prompt template](../guides/write-prompt-template.md) — use conditionals, iteration, and template functions to build production prompts.
- [WORKFLOW.md configuration reference](../reference/workflow-config.md) — every field, every default, every constraint.
- [Monitor with logs](../guides/monitor-with-logs.md) — understand the structured log output during long-running sessions.
- [Monitor with Prometheus](../guides/monitor-with-prometheus.md) — collect token usage, session counts, and retry rates as time-series metrics.
- [Use sub-agents with Sortie](../guides/use-subagents-with-sortie.md) — delegate work to specialized agents within a session.
- [Claude Code adapter reference](../reference/adapter-claude-code.md) — CLI flags, event stream, and pass-through configuration.
