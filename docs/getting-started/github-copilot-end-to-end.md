...
- [GitHub integration tutorial](github-integration.md) completed — Sortie connects to your GitHub repository and `SORTIE_GITHUB_TOKEN` is set
...
- [Write a prompt template](../guides/write-prompt-template.md) — conditionals, iteration, and template functions for production prompts
- [WORKFLOW.md configuration reference](../reference/workflow-config.md) — every field, every default, every constraint
- [Monitor with Prometheus](../guides/monitor-with-prometheus.md) — token usage, session counts, and retry rates as time-series metrics
- [Copilot CLI adapter reference](../reference/adapter-copilot.md) — CLI flags, event stream, and pass-through configuration
- [GitHub adapter reference](../reference/adapter-github.md) — field mapping, state derivation, and rate limiting
- [Scale agents with SSH](../guides/scale-agents-with-ssh.md) — remote execution for production workloads
