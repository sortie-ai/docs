---
title: "Claude Code Adapter | Sortie"
...
See also: [WORKFLOW.md configuration](workflow-config.md) for the full `agent` schema, [environment variables](environment.md#agent-runtime-variables) for `ANTHROPIC_API_KEY` and provider routing, [error reference](errors.md#agent-errors) for all agent error kinds, [how to write a prompt template](../guides/write-prompt-template.md) for template authoring.

---

## Configuration

The adapter reads from two configuration sections in [WORKFLOW.md front matter](workflow-config.md): the generic `agent` block (shared by all adapters) and the `claude-code` extension block (pass-through to the Claude Code CLI).
...
See [environment variables reference](environment.md#agent-runtime-variables) for the full list.
...
- [WORKFLOW.md configuration reference](workflow-config.md) — full `agent` schema and `claude-code` extension block
- [Environment variables reference](environment.md#agent-runtime-variables) — `ANTHROPIC_API_KEY`, Bedrock, Vertex AI, and proxy variables
