---
title: "Copilot CLI Adapter | Sortie"
...
See also: [WORKFLOW.md configuration](workflow-config.md) for the full `agent` schema, [environment variables](environment.md) for GitHub token variables, [error reference](errors.md#agent-errors) for all agent error kinds, [how to write a prompt template](../guides/write-prompt-template.md) for template authoring.

---

## Configuration

The adapter reads from two configuration sections in [WORKFLOW.md front matter](workflow-config.md): the generic `agent` block (shared by all adapters) and the `copilot-cli` extension block (pass-through to the Copilot CLI).
...
|---|---|---|
| `StrictHostKeyChecking` | Configurable (default: `accept-new`) | Host key verification policy. Set via [`worker.ssh_strict_host_key_checking`](workflow-config.md#worker). Allowed values: `accept-new`, `yes`, `no`. |
...
For Claude Code configuration, see [Claude Code adapter reference](adapter-claude-code.md).

---

## Related pages

- [WORKFLOW.md configuration reference](workflow-config.md) — full `agent` schema and `copilot-cli` extension block
- [Environment variables reference](environment.md) — GitHub token variables
- [Error reference](errors.md#agent-errors) — all agent error kinds with retry behavior
- [How to write a prompt template](../guides/write-prompt-template.md) — template variables, conditionals, and built-in functions
- [How to scale agents with SSH](../guides/scale-agents-with-ssh.md) — remote execution setup and host pool configuration
- [How to use the file adapter for local testing](../guides/use-file-adapter-for-testing.md) — test prompts without a live tracker
- [State machine reference](state-machine.md) — orchestration states, turn lifecycle, and stall detection
- [Dashboard reference](dashboard.md) — live monitoring of running sessions and token usage
- [Prometheus metrics reference](prometheus-metrics.md) — `sortie_agent_turns_total` and related counters
- [Agent extensions reference](agent-extensions.md) — `tracker_api` tool available during agent sessions
- [Claude Code adapter reference](adapter-claude-code.md) — sibling adapter for comparison
