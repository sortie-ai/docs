...
See also: [WORKFLOW.md configuration](workflow-config.md) for the full tracker schema, [how to connect Sortie to Jira Cloud](../guides/connect-to-jira.md) for setup instructions, [error reference](errors.md) for all tracker error kinds, [environment variables](environment.md) for `$VAR` expansion behavior.

---

## Configuration

The adapter reads its configuration from the `tracker` section of the [WORKFLOW.md front matter](workflow-config.md). Three fields are required; the rest have defaults.
...
Accepts [`$VAR` indirection](environment.md#var-indirection-in-workflowmd) via `resolveEnvRef` — the entire value must be a variable reference for expansion to apply.
...
Accepts [`$VAR` indirection](environment.md#var-indirection-in-workflowmd) via `resolveEnv` — variable references are expanded anywhere in the string.
...
The adapter implements all seven methods of the [`TrackerAdapter` interface](workflow-config.md). Each method maps to one or more Jira REST API v3 endpoints.
...
The adapter normalizes Jira API responses to [`domain.Issue`](workflow-config.md) fields. This table shows the exact mapping.
...
When the HTTP server is [enabled](workflow-config.md), the adapter increments the `sortie_tracker_requests_total` Prometheus counter for each API call.
...
- [How to connect Sortie to Jira Cloud](../guides/connect-to-jira.md) — setup instructions with authentication, state mapping, and troubleshooting
- [WORKFLOW.md configuration reference](workflow-config.md) — full schema for the `tracker` section and all other configuration
- [Error reference](errors.md#tracker-errors) — all tracker error kinds with retry behavior and operator actions
- [Environment variables reference](environment.md) — `$VAR` expansion modes and agent passthrough variables
- [Prometheus metrics reference](prometheus-metrics.md) — `sortie_tracker_requests_total` and related counters
- [How to write a prompt template](../guides/write-prompt-template.md) — using `.issue` fields (populated by this adapter) in templates
- [Agent extensions reference](agent-extensions.md) — `tracker_api` tool that agents use to call back into the tracker
- [How to use the file adapter for local testing](../guides/use-file-adapter-for-testing.md) — test prompts and hooks without Jira API credentials
- [State machine reference](state-machine.md) — orchestration states, candidate eligibility, and how tracker state drives dispatch
- [Dashboard reference](dashboard.md) — live monitoring of issues fetched by this adapter
