...
See also: [agent communication model](../concepts/agent-communication.md) for why two channels exist, [environment variables reference](environment.md#mcp-server-environment) for MCP server environment, [WORKFLOW.md configuration](workflow-config.md) for the `agent` section.
...
| `max_turns` | integer | Configured [`agent.max_turns`](workflow-config.md). |
...
For detailed patterns and worked examples, see [how to use agent tools in prompts](../guides/use-agent-tools-in-prompts.md).

---

## See also

- [Agent communication model](../concepts/agent-communication.md) — why two channels (file protocol + MCP tools) exist
- [How to use agent tools in prompts](../guides/use-agent-tools-in-prompts.md) — task-specific tool guidance for workflow authors
- [How to write a custom agent tool](../guides/write-custom-agent-tool.md) — implementing the `Tool` interface
- [Environment variables reference](environment.md#mcp-server-environment) — MCP server env vars
- [WORKFLOW.md configuration reference](workflow-config.md) — `agent` section, `agent.max_turns`
- [Error reference](errors.md) — tracker error kinds with retry behavior
- [State machine reference](state-machine.md) — orchestration states, retry suppression
- [Prometheus metrics reference](prometheus-metrics.md) — `sortie_tool_calls_total` counter
