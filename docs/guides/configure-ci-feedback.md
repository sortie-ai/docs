---
title: "How to Configure CI Feedback | Sortie"
...
- Sortie running with the GitHub tracker adapter (`tracker.kind: github`) — see [Connect to GitHub](connect-to-github.md)
- A branch-per-issue hook workflow that pushes commits — see [Setup workspace hooks](setup-workspace-hooks.md)
...
For more on template syntax, see [Write a prompt template](write-prompt-template.md).
...
If the agent signals `blocked` via `.sortie/status` during a CI-fix run, the orchestrator respects that signal and releases the claim without further CI checks. For details on the agent-to-orchestrator protocol, see the [agent extensions reference](../reference/agent-extensions.md).
...
For the full WORKFLOW.md configuration reference including all sections, see [workflow config reference](../reference/workflow-config.md).

## Related guides

- [Configure retry behavior](configure-retry-behavior.md) — `max_sessions`, backoff, stall detection
- [Connect to GitHub](connect-to-github.md) — GitHub adapter setup, token scopes
- [Setup workspace hooks](setup-workspace-hooks.md) — hook scripts, environment variables
- [Write a prompt template](write-prompt-template.md) — template syntax, `{{ .ci_failure }}` variable
- [Agent extensions reference](../reference/agent-extensions.md) — `.sortie/status` protocol
- [State machine reference](../reference/state-machine.md) — claim lifecycle, retry states
- [Prometheus metrics reference](../reference/prometheus-metrics.md) — CI-related metrics
- [Error reference](../reference/errors.md) — CI error kinds
