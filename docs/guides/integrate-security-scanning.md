---
title: "How to Integrate Security Scanning | Sortie"
description: "Run gitleaks, semgrep, govulncheck, and other security scanners in Sortie workspace hooks to catch secrets, vulnerabilities, and code issues before agent code reaches a PR."
...
# How to integrate security scanning into agent workflows
...
- Sortie up and running with at least one workspace configured ([quick start](../getting-started/quick-start.md))
- Hooks configured for your workspace ([set up workspace hooks](setup-workspace-hooks.md))
...
For how a blocked `before_run` interacts with retry budgets and backoff timing, see [Configure retry behavior](configure-retry-behavior.md).
...
For all available hook configuration fields, see the [workflow config reference](../reference/workflow-config.md).
...
- [Set up workspace hooks](setup-workspace-hooks.md) — hook lifecycle and environment variables
- [Configure retry behavior](configure-retry-behavior.md) — what happens when `before_run` exits non-zero
- [Security model](../concepts/security.md) — Sortie's trust boundaries and what the operator is responsible for
- [Errors reference](../reference/errors.md) — how hook failures are classified in error logs
