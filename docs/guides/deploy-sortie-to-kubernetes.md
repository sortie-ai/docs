---
title: "How to Deploy Sortie to Kubernetes | Sortie"
...
- An agent-specific container image pushed to a registry your cluster can pull from ([how to build one](use-sortie-in-docker.md))
- A tested `WORKFLOW.md` ([quick start](../getting-started/quick-start.md))
...
For image building details, see [How to use Sortie in Docker](use-sortie-in-docker.md).
...
For tracker-specific credential details, see [How to connect to Jira](connect-to-jira.md) or [How to connect to GitHub Issues](connect-to-github.md).
...
For the full list of configuration fields, see the [WORKFLOW.md configuration reference](../reference/workflow-config.md). For prompt template syntax, see [How to write prompt templates](write-prompt-template.md).
...
For background on what Sortie persists and why it matters, see [Why persistence changes everything](../concepts/persistence.md).
...
| `--log-format json` | Produces newline-delimited JSON for log aggregation. See [How to monitor with logs](monitor-with-logs.md). |
...
This Service gives in-cluster access to the [HTML dashboard](../reference/dashboard.md), [JSON API](../reference/http-api.md), [Prometheus metrics](../reference/prometheus-metrics.md), and health probes. To expose the dashboard externally, add an Ingress or LoadBalancer in front of it.
...
Open `http://localhost:7678` to view the [dashboard](../reference/dashboard.md), or query the API:
...
The Deployment controller recreates it. Check the logs for a warm-start message indicating that existing state was loaded. In-flight agent sessions that were interrupted are marked as timed-out and retried according to your [retry configuration](configure-retry-behavior.md).

For a deeper look at what Sortie preserves across restarts, see [How to resume sessions across restarts](resume-sessions-across-restarts.md).

## Monitor the deployment

### Prometheus

If you run Prometheus in the cluster, add a scrape target or ServiceMonitor for the `sortie` Service on port 7678 at the `/metrics` endpoint. See [How to monitor with Prometheus](monitor-with-prometheus.md) for PromQL queries and a Grafana dashboard.
...
See [How to monitor with logs](monitor-with-logs.md) for field descriptions and grep/jq patterns.
...
If your cluster enforces Pod Security Standards, the manifest complies with the `restricted` profile. See [Security model](../concepts/security.md) for Sortie's workspace isolation guarantees.
