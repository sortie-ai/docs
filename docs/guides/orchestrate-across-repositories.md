---
title: "How to Orchestrate Agents Across Multiple Repositories | Sortie"
...
- Multiple workflows already working ([run multiple workflows](run-multiple-workflows.md))
- Workspace hooks configured ([set up workspace hooks](setup-workspace-hooks.md))
- A connected tracker: [GitHub Issues](connect-to-github.md) or [Jira Cloud](connect-to-jira.md)
...
For isolation rules and concurrency accounting across multiple processes, see [run multiple workflows](run-multiple-workflows.md).
...
See [connect to Jira](connect-to-jira.md) for full JQL filter syntax.
...
Centralized tracking is easier to oversee: one backlog, one board. Distributed tracking is simpler per-instance but requires switching between repos to see the full picture. Pick the model your team already uses. See [connect to GitHub](connect-to-github.md) for label and search syntax details.

## Configure per-repo hooks

Each repository needs its own clone and branch setup. This is where the multi-repo pattern diverges from [single-workflow hooks](setup-workspace-hooks.md).
...
All instances expose `/metrics` on their respective ports. A single Prometheus scrape config with multiple targets collects from all of them. See [monitor with Prometheus](monitor-with-prometheus.md) for the multi-target configuration.
