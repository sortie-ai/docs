---
title: "How to Resume Agent Sessions Across Restarts | Sortie"
...
For deeper coverage of hook patterns, see [Set Up Workspace Hooks](setup-workspace-hooks.md).
...
For more on reading Sortie's logs, see [Monitor with Logs](monitor-with-logs.md).

### Use the dashboard

The built-in dashboard reads directly from SQLite. Run history, active sessions, and pending retries all reflect persisted state — they survive restarts along with everything else. See the [Dashboard reference](../reference/dashboard.md).
...
- [Configure Retry Behavior](configure-retry-behavior.md) — tune backoff timing and session budgets
- [Set Up Workspace Hooks](setup-workspace-hooks.md) — hook writing patterns and lifecycle details
- [Run as a systemd Service](run-as-systemd-service.md) — automatic restart on failure with `Restart=on-failure`
- [Monitor with Logs](monitor-with-logs.md) — filter and interpret startup recovery messages
