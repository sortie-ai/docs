---
title: How to Troubleshoot Common Failures | Sortie
...
Each section below covers one failure — the log line you see, why it happens, and what to do. For the full error catalog with every error kind and retry formula, see the [error reference](../reference/errors.md).
...
2. For AWS Bedrock or Google Vertex AI, verify all required variables are set. See [environment variables reference](../reference/environment.md) for the full list.
...
- **Typo in a field name.** Check the name against the [variable table](write-prompt-template.md#use-all-available-issue-fields). The error message names the exact field and line.
...
See the [workflow configuration reference](../reference/workflow-config.md) for every field, default, and constraint.
